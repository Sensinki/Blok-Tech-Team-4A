...
const templateData = require("./public/js/gamestemplaterosalie");
>>>>>>> b5d2d4ec040cbfd181d571d465ca6837536492d3
...
// Definieer de route voor '/like'
app.get("/like", checkSession, (req, res) => {
  const games = templateData.games;
  res.render("like", { games: games });
});

//zoiets kan je doen om de ganes io te halen
app.get("/like", checkSession,  (req, res) => {
  Game.find()
    .then((games) => {
      res.render("like", { games: games });
    ...
    .catch((error) => {
      console.error("Gaat even niet zo goed:", error);
      res.sendStatus(500);
    });
});
